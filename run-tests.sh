...
if [ -n "${CIRCLE_PR_NUMBER}" ]; then
  sed -i .bak 's/syndesis-e2e-tests/syndesis-e2e-tests-pr${CIRCLE_PR_NUMBER}/g' test.properties
else
  sed -i .bak 's/syndesis-e2e-tests/syndesis-e2e-tests-smoke/g' test.properties
fi

./mvnw clean test -B -P all -Dcucumber.options="--tags ${CUCUMBER_TAG}"
